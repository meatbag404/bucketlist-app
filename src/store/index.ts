...
import {
  ...
  BucketWithMembers, Activity, FriendProfile
} from '../types/database'
...
interface AppState {
  ...
  // Pending approvals (for bucket owners)
  ...
  editItem: (itemId: string, updates: { title?: string; memory_note?: string; target_date?: string; category_key?: string; emoji?: string; location?: string }) => Promise<void>
...
export const useStore = create<AppState>((set, get) => ({
  ...
  fetchBuckets: async () => {
    ...
    // Deduplicate — RLS returns one row per member per bucket
    ...
    // Refresh counts whenever buckets reload
  ...
  markItemDone: async (itemId) => {
    ...
    if (!error) {
      ...
      if (bucket && profile && item) {
        ...
        // Push notification to bucket members
  ...
  leaveBucket: async (bucketId) => {
    ...
    await supabase
      ...
      .eq('bucket_id', bucketId)
      .eq('user_id', profile.id)
    // Switch to another bucket or clear
  ...
  addFriendToBucket: async (friendId, bucketId) => {
    ...
    if (existing?.status === 'pending') {
      // Auto-approve their pending request since owner is explicitly adding them
      const { error } = await supabase
  ...
  reorderItems: async (reorderedTodo) => {
    // Optimistic update — splice new todo order in, keep done items at the end
    const doneItems = get().items.filter(i => i.done)
    set({ items: [...reorderedTodo, ...doneItems] })

    // Persist new sort_order values (spaced by 1000 for future insertions)
    await Promise.all(
  ...
  registerPushToken: async () => {
    ...
    try {
      ...
      console.error('Push token registration:', e)
    }
  },
...
export const useFilteredItems = () => {
  ...
  // Starred items float to the top (within todo section only)
  return [...filtered].sort((a, b) => {
    if (a.done !== b.done) return 0 // don't reorder across done/todo boundary
